...
type Obj struct {
	Value          interface{} // A box that can contain evey values
...
func (d *Dict) Set(k string, obj *Obj) {
	if len(d.dictStore) == config.MAX_KEYS {
		d.evict()
	}
	v := d.dictStore[k]
	if v == nil {
...
func (d *Dict) evictRandom() {
	evictCount := int64(config.EVICTION_RATIO * float64(config.MAX_KEYS))
