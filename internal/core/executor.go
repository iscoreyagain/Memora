...
func cmdPING(args []string) []byte {
	var res []byte
	if len(args) > 1 {
		return protocol.Encode(errors.New("ERR wrong number of arguments for 'ping' command"), false)
	}

	if len(args) == 0 {
		res = protocol.Encode("PONG", true)
	} else {
		res = protocol.Encode(args[0], false)
	}
	return res
}
...
func cmdSET(args []string) []byte {
	if len(args) < 2 || len(args) == 3 || len(args) > 4 {
		return protocol.Encode(errors.New("(error) ERR wrong number of arguments for 'SET' command"), false)
	}
	...
	if len(args) > 2 {
		...
		if err != nil {
			return protocol.Encode(errors.New("(error) ERR value is not an integer or out of range"), false)
		}
		ttlMs = ttlSec * 1000
	}
...
func cmdGET(args []string) []byte {
	if len(args) != 1 {
		return protocol.Encode(errors.New("(error) ERR wrong number of arguments for 'GET' command"), false)
	}
	...
	return protocol.Encode(obj.Value, false)
}

func cmdTTL(args []string) []byte {
	if len(args) != 1 {
		return protocol.Encode(errors.New("(error) ERR wrong number of arguments for 'TTL' command"), false)
	}
	key := args[0]
	obj := dictStore.Get(key)
	if obj == nil {
	...
	return protocol.Encode(int64(remainMs/1000), false)
}
...
func cmdINFO(args []string) []byte {
	...
	return protocol.Encode(buf.String(), false)
}
